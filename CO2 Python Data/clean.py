...
# Load the dataset
data = """Year,Jan,Feb,Mar,Apr,May,Jun,Jul,Aug,Sep,Oct,Nov,Dec,J-D,D-N,DJF,MAM,JJA,SON
...
# Step 2: Convert appropriate columns to numeric types
for col in df.columns[1:]:  # Skip the 'Year' column
    df[col] = pd.to_numeric(df[col], errors='coerce')  # Converts to float, coerce errors to NaN

# Step 3: Check for any rows with all NaN values and drop them
df.dropna(how='all', inplace=True)

# Step 4: Optionally, check for duplicates and remove them
df.drop_duplicates(inplace=True)

# Step 5: Reset the index of the DataFrame
df.reset_index(drop=True, inplace=True)

# Step 6: Optionally, you can also save the cleaned data to a new CSV file
df.to_csv('cleaned_land_ocean_data.csv', index=False)
